...
if ARGV.empty?
  puts "Использование: ruby scripts/validate_90.rb <routing_decisions.json>"
  exit 1
end
...
# Находим файл с очередью на 90 заявок
queue = load_json('operations_queue_90.json') || load_json('operations_queue_test.json')
...
puts "=== Проверка routing_decisions.json (90 заявок) ==="
...
# 3. Детерминированные кейсы (если присутствуют в reference)
if reference && reference['deterministic_cases']
...
# 4. Eligible providers для всех 90 заявок
...
puts "✅ Все выбранные провайдеры входят в допустимый список (Hard Constraints пройдены)"
